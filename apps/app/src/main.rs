...
fn is_allowed_blockbench_skin_request(
    ...
    if !matches!(
        request.uri().host(),
        Some("localhost") | Some("wisp-skin.localhost")
    ) {
        return false;
    }
...
fn main() {
    // Initialize portable mode first (checks .WispLauncher folder and sets THESEUS_CONFIG_DIR)
    ...
    */
    ...
    tracing::info!("Initialized tracing subscriber. Loading WispLauncher!");
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                if format!("{e:?}").contains(
                    ...
                    DialogBuilder::message()
                        .set_level(MessageLevel::Error)
                        .set_title("Initialization error")
                        .set_text("Your Microsoft Edge WebView2 installation is corrupt.\n\nMicrosoft Edge WebView2 is required to run WispLauncher.\n\nRepair or reinstall the Microsoft Edge WebView2 Runtime, then start WispLauncher again.")
                        .alert()
                        .show()
                        .unwrap();
