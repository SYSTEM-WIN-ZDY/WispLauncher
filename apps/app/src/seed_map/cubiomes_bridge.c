...
static float wisp_clamp(float value, float low, float high);

static int wisp_map_approx_height(
	float *heights,
...
enum {
	WISP_FEATURE_VILLAGE = 1 << 0,
	WISP_FEATURE_OUTPOST = 1 << 1,
	WISP_FEATURE_SHIPWRECK = 1 << 2,
	WISP_FEATURE_MONUMENT = 1 << 3,
	WISP_FEATURE_MANSION = 1 << 4,
	WISP_FEATURE_ANCIENT_CITY = 1 << 5,
	WISP_FEATURE_TRAIL_RUINS = 1 << 6,
	WISP_FEATURE_TRIAL_CHAMBERS = 1 << 7,
	WISP_FEATURE_RUINED_PORTAL = 1 << 8,
	WISP_FEATURE_STRONGHOLD = 1 << 9,
	WISP_FEATURE_SLIME_CHUNK = 1 << 10,
	WISP_FEATURE_DESERT_PYRAMID = 1 << 11,
	WISP_FEATURE_JUNGLE_TEMPLE = 1 << 12,
	WISP_FEATURE_SWAMP_HUT = 1 << 13,
	WISP_FEATURE_IGLOO = 1 << 14,
	WISP_FEATURE_OCEAN_RUIN = 1 << 15,
	WISP_FEATURE_BURIED_TREASURE = 1 << 16,
	WISP_FEATURE_MINESHAFT = 1 << 17,
	WISP_FEATURE_DESERT_WELL = 1 << 18,
	WISP_FEATURE_GEODE = 1 << 19,
	WISP_FEATURE_FORTRESS = 1 << 20,
	WISP_FEATURE_BASTION = 1 << 21,
	WISP_FEATURE_END_CITY = 1 << 22,
	WISP_FEATURE_END_GATEWAY = 1 << 23,
};

typedef struct WispStructureDefinition {
	int type;
	uint32_t flag;
}WispStructureDefinition;

static const WispStructureDefinition WISP_STRUCTURES[] = {
	{Village, WISP_FEATURE_VILLAGE},
	{Outpost, WISP_FEATURE_OUTPOST},
	{Shipwreck, WISP_FEATURE_SHIPWRECK},
	{Monument, WISP_FEATURE_MONUMENT},
	{Mansion, WISP_FEATURE_MANSION},
	{Ancient_City, WISP_FEATURE_ANCIENT_CITY},
	{Trail_Ruins, WISP_FEATURE_TRAIL_RUINS},
	{Trial_Chambers, WISP_FEATURE_TRIAL_CHAMBERS},
	{Ruined_Portal, WISP_FEATURE_RUINED_PORTAL},
	{Ruined_Portal_N, WISP_FEATURE_RUINED_PORTAL},
	{Desert_Pyramid, WISP_FEATURE_DESERT_PYRAMID},
	{Jungle_Temple, WISP_FEATURE_JUNGLE_TEMPLE},
	{Swamp_Hut, WISP_FEATURE_SWAMP_HUT},
	{Igloo, WISP_FEATURE_IGLOO},
	{Ocean_Ruin, WISP_FEATURE_OCEAN_RUIN},
	{Treasure, WISP_FEATURE_BURIED_TREASURE},
	{Mineshaft, WISP_FEATURE_MINESHAFT},
	{Desert_Well, WISP_FEATURE_DESERT_WELL},
	{Geode, WISP_FEATURE_GEODE},
	{Fortress, WISP_FEATURE_FORTRESS},
	{Bastion, WISP_FEATURE_BASTION},
	{End_City, WISP_FEATURE_END_CITY},
	{End_Gateway, WISP_FEATURE_END_GATEWAY},
};

int32_t wisp_seed_map_java_version(int32_t version) {
	switch (version) {
		/*
...
static const unsigned char WISP_END_VOID[3] = {0x0D, 0x0D, 0x16};

static int wisp_smooth_height_grid(
	float *grid,
	int width,
	int height,
	int passes
...
static void wisp_render_end(
	...
	if (wisp_smooth_height_grid(heights, grid_width, grid_height, passes) != 0) {
		free(land);
		free(heights);
		return;
	}
	for (int row = 0; row < height; row++) {
		for (int column = 0; column < width; column++) {
			...
			if (contours && (surface / 16 != west / 16 || surface / 16 != north / 16)) {
				...
				if (slope != 0) {
					const float delta = wisp_clamp((float)slope / 16.0f, -0.3f, 0.3f);
			...
			for (int channel = 0; channel < 3; channel++) {
				pixel[channel] =
					(uint8_t)wisp_clamp((float)pixel[channel] * factor, 0.0f, 255.0f);
			}
		}
	}
	free(land);
 ...
 */
static void wisp_shade_height_map(
	uint8_t *rgb,
	int32_t width,
	int32_t height,
	int32_t contours,
	...
	for (int row = 0; row < height; row++) {
		...
		for (int column = 0; column < width; column++) {
			...
			const size_t index = (size_t)row * (size_t)width + (size_t)column;
			const int here =
				wisp_block_height(grid, grid_width, grid_height, cell_column, cell_row);
			const int west =
				wisp_block_height(grid, grid_width, grid_height, cell_column - 1, cell_row);
			const int north =
				wisp_block_height(grid, grid_width, grid_height, cell_column, cell_row - 1);
			const int northwest =
				wisp_block_height(grid, grid_width, grid_height, cell_column - 1, cell_row - 1);
			float factor = 1.0f;
			if (contours) {
			...
			for (int channel = 0; channel < 3; channel++) {
				...
				pixel[channel] = (uint8_t)wisp_clamp(shaded, 0.0f, 255.0f);
			}
		}
	}
}
 ...
 */
static int wisp_fill_sparse_height_grid(
	const Generator *generator,
	const SurfaceNoise *surface_noise,
	int64_t min_block_x,
	int64_t min_block_z,
...
int wisp_seed_map_render(
	...
	if (terrain && dimension == DIM_OVERWORLD) {
		...
		if (scale <= 4) {
			...
			if (heights) {
				if (wisp_fill_sparse_height_grid(
...
static int8_t wisp_end_city_has_ship(uint64_t seed, int block_x, int block_z) {
...
size_t wisp_seed_map_find_features(
	uint64_t seed,
	int32_t minecraft_version,
	int32_t generator_flags,
	int32_t dimension,
	...
	size_t count = 0;
	for (size_t index = 0; index < sizeof(WISP_STRUCTURES) / sizeof(WISP_STRUCTURES[0]); index++) {
		const WispStructureDefinition definition = WISP_STRUCTURES[index];
		...
		for (int region_x = min_region_x; region_x <= max_region_x; region_x++) {
			for (int region_z = min_region_z; region_z <= max_region_z; region_z++) {
				...
				const int8_t end_ship = definition.type == End_City
					? wisp_end_city_has_ship(seed, position.x, position.z)
					: -1;
				if (!add_feature(
	...
	if (dimension == DIM_OVERWORLD && feature_mask & WISP_FEATURE_STRONGHOLD) {
		...
		for (int index = 0; index < 32; index++) {
			...
			if (stronghold.pos.x >= min_x && stronghold.pos.x <= max_x && stronghold.pos.z >= min_z && stronghold.pos.z <= max_z) {
				if (!add_feature(
						...
						WISP_FEATURE_STRONGHOLD, 0, -1)) return count;
			}
		}
	}

	if (dimension == DIM_OVERWORLD && feature_mask & WISP_FEATURE_SLIME_CHUNK) {
		...
		for (int chunk_x = min_chunk_x; chunk_x <= max_chunk_x; chunk_x++) {
			for (int chunk_z = min_chunk_z; chunk_z <= max_chunk_z; chunk_z++) {
				...
				if (!add_feature(
						...
						WISP_FEATURE_SLIME_CHUNK, 0, -1)) return count;
			}
		}
	}
...
int32_t wisp_seed_map_biome_at(
	uint64_t seed,
	int32_t minecraft_version,
	int32_t generator_flags,
	int32_t dimension,
 ...
 */
static void wisp_init_vein_noise(WispVeinNoise *vein_noise, uint64_t seed) {
...
static uint64_t wisp_block_position_seed(int32_t x, int32_t y, int32_t z) {
...
static Xoroshiro wisp_vein_block_rng(const WispVeinNoise *vein_noise, int32_t x, int32_t y, int32_t z) {
	Xoroshiro xr;
	xr.lo = wisp_block_position_seed(x, y, z) ^ vein_noise->ore_lo;
...
static double wisp_clamped_map(double value, double from_min, double from_max, double to_min, double to_max) {
 ...
 */
size_t wisp_seed_map_scan_vein(
	uint64_t seed,
	int32_t chunk_x,
	int32_t chunk_z,
	int32_t vein_kind,
	...
	if (!out_xyz || out_cap == 0) return 0;
	 WispVeinNoise vein_noise;
	wisp_init_vein_noise(&vein_noise, seed);
	...
	for (int y = min_y; y <= max_y && count < out_cap; y += 2) {
		...
		for (int dx = 0; dx < 16 && count < out_cap; dx += 2) {
			for (int dz = 0; dz < 16 && count < out_cap; dz += 2) {
				...
				Xoroshiro block_rng = wisp_vein_block_rng(&vein_noise, x, y, z);
				...
				const double richness =
					wisp_clamped_map(toggle_signed, 0.4, 0.6, 0.1, 0.3);
...
int wisp_seed_map_surface_heights(
	uint64_t seed,
	int32_t minecraft_version,
	int32_t generator_flags,
	int32_t x,
