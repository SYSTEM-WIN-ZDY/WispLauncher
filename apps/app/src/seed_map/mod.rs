...
unsafe extern "C" {
    fn wisp_seed_map_java_version(version: i32) -> i32;
    fn wisp_seed_map_render(
        seed: u64,
    ...
    fn wisp_seed_map_find_features(
        seed: u64,
        minecraft_version: i32,
        generator_flags: i32,
        dimension: i32,
    ...
    fn wisp_seed_map_surface_heights(
        seed: u64,
        minecraft_version: i32,
        generator_flags: i32,
        x: i32,
...
/*
 * Supported Java versions, newest first, using Wisp version codes
...
pub(crate) fn resolve_java_version(version: &str) -> io::Result<(i32, i32)> {
    ...
    let engine_version = unsafe { wisp_seed_map_java_version(code) };
    if engine_version == 0 {
        return Err(io::Error::other(
...
mod tests {
    ...
    fn write_preview_tiles() {
        let Some(dir) = std::env::var_os("WISP_SEED_MAP_PREVIEW_DIR") else {
            return;
        };
