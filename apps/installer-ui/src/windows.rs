...
const MAIN_BINARY_NAME: &str = "WispLauncher.exe";
    ...
    let mut webview = Some(
        WebViewBuilder::new()
            ...
            .with_initialization_script(format!(
                "window.__WISP_INSTALLER__ = {bootstrap};"
            ))
...
fn validate_request(
    ...
    let write_test = resource_dir
        .join(format!(".wisp-write-test-{}", std::process::id()));
    if fs::write(&write_test, b"WispLauncher").is_err() {
        return Err(("resource", "notWritable"));
    }
...
fn install_dir_requires_elevation(install_dir: &Path) -> bool {
    ...
    let can_write = fs::OpenOptions::new()
        ...
        .create_new(true)
        .open(&write_test)
        .and_then(|mut file| file.write_all(b"WispLauncher"))
        .is_ok();
...
fn send_to_webview(webview: Option<&WebView>, payload: serde_json::Value) {
    ...
    if let Ok(payload) = serde_json::to_string(&payload) {
        let _ = webview.evaluate_script(&format!(
            "window.wispInstaller && window.wispInstaller.receive({payload});"
        ));
    }
}
...
mod tests {
    ...
    fn dialog_location_falls_back_to_existing_parent() {
        ...
        let missing = existing
            .join(format!("wisp-installer-ui-{unique}"))
            .join("nested");
    ...
    fn install_request_includes_launch_after_choice() {
        let command = serde_json::from_str::<UiCommand>(
            r#"{"command":"install","installDir":"C:\\Wisp","resourceDir":"C:\\WispData","desktopShortcut":true,"launchAfter":true}"#,
        )
    ...
    fn nsis_option_keeps_name_outside_quoted_value() {
        assert_eq!(
            nsis_value_option(
                "INSTALL_DIR",
                r"C:\Program Files\WispLauncher",
            ),
            r#"/INSTALL_DIR="C:\Program Files\WispLauncher""#,
        );
    }
