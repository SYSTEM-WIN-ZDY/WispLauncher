...
// Some WebKitGTK builds can drop blob: URLs (garbage-collected or invalidated
// between renders), which makes <img> fail right after a re-render and the
// avatar suddenly disappear. Use a self-contained data: URL instead so the
// avatar never needs the browser to keep the blob alive.
...
async function loadHeadRender(skin: Skin, headKey: string): Promise<string> {
	if (!DEBUG_MODE) {
		try {
			// Guard against IndexedDB hanging in some WebKitGTK builds.
			const cachedBlob = await Promise.race([
				headStorage.retrieveBlob(headKey),
				new Promise<Blob | null>((resolve) => setTimeout(() => resolve(null), 3000)),
			])
			if (cachedBlob) {
	...
	const headUrl = await blobToDataUrl(headBlob)
