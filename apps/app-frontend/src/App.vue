...
import WispLogo from '@/components/ui/WispLogo.vue'
...
import { WispBrandConfig, config, getOfficialLabrinthBaseUrl } from '@/config'
...
const tauriApiClient = new TauriModrinthClient({
	userAgent: async () =>WispBrandConfig.userAgent(await appVersion, await getOsType()),
	labrinthBaseUrl: config.labrinthBaseUrl,
	features: [
		...(WispBrandConfig.capabilities.privateModrinthServices
			? [
					new AuthFeature({
...
const messages = defineMessages({
	...
	runningAsAdmin: {
		...
		defaultMessage:
			'WispLauncher is running as administrator. Drag-and-drop file import is disabled in this mode; please restart the launcher without administrator privileges.',
	},
	restarting: {
		id: 'app.restarting',
...
setupAuthProvider(credentials, async (_redirectPath) => {
	if (WispBrandConfig.capabilities.privateModrinthServices) await signIn()
})
...
async function fetchCredentials() {
	if (!WispBrandConfig.capabilities.privateModrinthServices) {
...
const updatePopupMessages = defineMessages({
	...
	meteredBody: {
		...
		defaultMessage: `WispLauncher v{version} is available now! Since you're on a metered network, we didn't automatically download it.`,
	},
	downloadedBody: {
		id: 'app.update-popup.body.download-complete',
		defaultMessage: `WispLauncher v{version} has finished downloading. Reload to update now, or automatically when you close WispLauncher.`,
	},
	reload: {
...
setAppUpdateActions({
	...
	changelog: (version) => {
		if (version && getAnnouncementByVersion(version)) {
			...
			openUrl(WispBrandConfig.website)
		}
	},
})
...
<template>
	...
	>
		...
		>
			...
			<OverflowMenu
				v-if="WispBrandConfig.capabilities.privateModrinthServices && credentials?.user"
			...
			<NavButton
				v-else-if="WispBrandConfig.capabilities.privateModrinthServices"
		...
		<div data-tauri-drag-region class="app-grid-statusbar bg-bg-raised h-[--top-bar-height] flex">
			<div data-tauri-drag-region class="flex min-w-0 flex-1 overflow-hidden p-3">
				<WispLogo class="h-full w-auto shrink-0 pointer-events-none" />
