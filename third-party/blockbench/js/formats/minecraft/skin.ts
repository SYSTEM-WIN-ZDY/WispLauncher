	...
	form: {
		...
		texture_source: {
			...
			options: {
				...
				load_texture: navigator.onLine && !Blockbench.queries?.embed ? 'dialog.skin.texture_source.load_texture' : undefined,
...
BARS.defineActions(function() {
	...
	new Action('export_minecraft_skin', {
		...
		click: function () {
			if (Blockbench.queries?.embed) {
				let texture = Texture.all[0];
				window.parent.postMessage({
					type: 'axolotl-skin-export',
					name: texture.name.endsWith('.png') ? texture.name : `${texture.name}.png`,
