...
impl ZipEntrySet {
    ...
    /// Whether `wisp_config.json` exists under `base`.
    fn has_wisp_config(&self, base: &str) -> bool {
        self.has_file(base, "wisp_config.json")
    }

    /// Whether a direct child folder under `base` has `wisp_config.json`
    /// (a container of WispLauncher instances).
    fn has_direct_wisp_config(&self, base: &str) -> bool {
        self.child_folders(base).iter().any(|child| {
            self.has_file(&format!("{base}{child}/"), "wisp_config.json")
        })
    }
...
fn classify_zip_entries<R: std::io::Read + std::io::Seek>(
    ...
    //    worlds.
    let launcher_type = if entries.has_wisp_config(base)
        || entries.has_direct_wisp_config(base)
    {
        Some(ImportLauncherType::WispLauncher)
...
fn classify_folder_content_inner(
    ...
    if is_wisp_folder(path) {
        return DroppedItemType::Launcher {
            launcher_type: ImportLauncherType::WispLauncher,
...
/// Whether `path` is an WispLauncher instance root or a container whose direct
/// child folders are WispLauncher instances.
fn is_wisp_folder(path: &Path) -> bool {
    path.join("wisp_config.json").is_file()
        || has_direct_wisp_config(path)
}

/// Whether a direct child folder of `path` has `wisp_config.json`.
fn has_direct_wisp_config(path: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(path) else {
        return false;
    };
    entries.flatten().any(|entry| {
        let child = entry.path();
        child.is_dir() && child.join("wisp_config.json").is_file()
    })
}
