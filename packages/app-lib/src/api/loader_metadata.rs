...
const INSTALLER_URL_KEY: &str = "WISP_INSTALLER_URL";
const INSTALLER_ENTRY_KEY_PREFIX: &str = "WISP_INSTALLER_ENTRY_";
...
fn parse_installer(
    ...
    for (index, (key, side, archive_path)) in requests.into_iter().enumerate() {
        ...
        let coordinate = format!(
            "com.wisp.loader-installer:embedded:{}:{}-{}@{}",
...
mod tests {
    ...
    fn installer_parser_merges_profile_and_embedded_artifacts() {
        ...
        assert!(data["BINPATCH"].client.starts_with("[com.wisp."));
        assert_eq!(
