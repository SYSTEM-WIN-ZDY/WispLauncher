...
impl fmt::Display for ImportLauncherType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ImportLauncherType::WispLauncher => write!(f, "WispLauncher"),
...
pub async fn get_importable_instances(
    ...
    let instances = match launcher_type {
        ImportLauncherType::WispLauncher => get_wisp_instances(&base_path).await,
        ImportLauncherType::ModrinthApp => {
...
/// Collects WispLauncher instances from a base path. A base path holding
/// `wisp_config.json` is itself an instance; otherwise direct child
/// folders with their own config are treated as instances.
async fn get_wisp_instances(
    base_path: &Path,
) -> crate::Result<Vec<ImportableInstance>> {
    if base_path.join("wisp_config.json").is_file() {
        let name = base_path
            .file_name()
...
async fn import_via_launcher(
    ...
    match launcher_type {
        ...
        ImportLauncherType::WispLauncher => {
            wisp::import_wisp(
                instance_path.clone(),
                instance_id,
                reporter.clone(),
...
pub fn get_default_launcher_path(
    ...
    let path = match r#type {
        ...
        ImportLauncherType::WispLauncher => None,
...
pub async fn is_valid_importable_instance(
    ...
    match r#type {
        ...
        ImportLauncherType::WispLauncher => {
            instance_path.join("wisp_config.json").is_file()
        }
        ImportLauncherType::PCL2
        | ImportLauncherType::PCL2CE
        | ImportLauncherType::HMCL
