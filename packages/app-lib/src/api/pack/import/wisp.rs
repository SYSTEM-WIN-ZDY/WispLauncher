...
const CONFIG_FILE_NAME: &str = "wisp_config.json";
const CONFIG_SCHEMA_VERSION: u32 = 1;

#[derive(Deserialize, Clone, Debug)]
#[serde(rename_all = "snake_case")]
pub struct WispConfigFile {
    ...
    pub content_set: WispContentSet,
...
/// Imports an WispLauncher instance by reading `wisp_config.json` and applying
...
pub(crate) async fn import_wisp(
    source_path: PathBuf,
    instance_id: &str,
    ...
    let content = io::read_any_encoding_to_string(&config_path)
        ...
        .unwrap_or_else(|error| {
            tracing::warn!(
                "WispLauncher import: could not read {}: {error}; falling back to generic import",
                config_path.display()
            );
    ...
    let config = match serde_json::from_str::<WispConfigFile>(&content) {
        Ok(config) => config,
        Err(error) => {
            tracing::warn!(
                "WispLauncher import: invalid config {}: {error}; falling back to generic import",
                config_path.display()
            );
    ...
    tracing::debug!(
        "WispLauncher import: config instance_id={} path={} generated_at={} symlink_target={:?}",
        config.instance_id,
        config.path,
        config.generated_at,
    ...
    {
        tracing::warn!(
            "WispLauncher import: unsupported schema or missing game version for {}; falling back to generic import",
            config_path.display()
        );
        return generic::import_generic(
            source_path,
