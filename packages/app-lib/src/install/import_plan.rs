...
#[serde(rename_all = "snake_case")]
struct WispConfigMetadata {
    schema_version: Option<u32>,
    #[serde(default)]
    content_set: WispContentSetMetadata,
}

#[derive(Debug, Clone, Default, Deserialize)]
#[serde(rename_all = "snake_case")]
struct WispContentSetMetadata {
    #[serde(default)]
...
fn detect_import_plan_info(
    ...
    let config_info = read_wisp_config(source);
    DetectedInstanceInfo {
...
fn read_wisp_config(source: &Path) -> Option<WispConfigMetadata> {
    let path = source.join("wisp_config.json");
    if !path.is_file() {
        return None;
    }
    let content = std::fs::read_to_string(&path).ok()?;
    let config: WispConfigMetadata = serde_json::from_str(&content).ok()?;
...
async fn scan_import_plan(
    ...
    if detected_loader.as_deref().is_some_and(|loader| {
        ...
        return Err(crate::ErrorKind::InputError(
            "Unsupported loader LabyMod: WispLauncher does not install, update, or repair LabyMod instances"
                .to_string(),
        )
        .into());
    }
