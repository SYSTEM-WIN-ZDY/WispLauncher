    ...
    match job_state.request.clone() {
        ...
        InstallRequest::ImportInstance {
            ...
            crate::api::pack::import::import_instance_with_reporter(
                ...
                // generic/PCL/HMCL/WispLauncher paths already consume them.
                InstallProgressReporter::new(job_id, job_state.clone()),
                symlink,
            )
            .await?;
...
async fn apply_upgrade_content(
    ...
    #[cfg(debug_assertions)]
    tracing::warn!(
        raw_env = ?std::env::var("WISP_TEST_UPGRADE_FAIL_AFTER_MUTATIONS"),
    ...
    #[cfg(debug_assertions)]
    tracing::warn!(
        raw_env = ?std::env::var("WISP_TEST_UPGRADE_PAUSE_AFTER_MUTATIONS"),
...
fn injected_upgrade_failure_after_mutations() -> Option<usize> {
    std::env::var("WISP_TEST_UPGRADE_FAIL_AFTER_MUTATIONS")
        .ok()
...
fn injected_upgrade_pause_after_mutations() -> Option<usize> {
    std::env::var("WISP_TEST_UPGRADE_PAUSE_AFTER_MUTATIONS")
        .ok()
        .and_then(|value| debug_mutation_count(&value))
}
