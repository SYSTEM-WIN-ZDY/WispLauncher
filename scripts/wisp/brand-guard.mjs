...
const requiredInvariants = [
	['product name', tauriConfig.productName === 'WispLauncher'],
	['bundle identifier', tauriConfig.identifier === 'red.ghs.wisp'],
	[
		'deep-link scheme',
		tauriConfig.plugins?.['deep-link']?.desktop?.schemes?.includes('wisp') === true,
	],
	[
		'User-Agent format',
		frontendConfig.includes('garbage-human-studio/wisp/${version} (${os})'),
	],
	[
...
if (failures.length > 0) {
	console.error(` Wisp brand guard failed:\n${failures.join('\n')}`)
	process.exit(1)
}

console.log(' Wisp brand guard passed.')
